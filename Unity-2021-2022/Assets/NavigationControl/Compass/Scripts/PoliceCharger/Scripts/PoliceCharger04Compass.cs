/*
*  Name: Police Charger 04 Compass
*  File: PoliceCharger04Compass.cs
...
namespace NavigationControl
{
	// public class PoliceCharger04Compass
	public class PoliceCharger04Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The PoliceCharger04 transform")]
		...
		public static PoliceCharger04Compass _charger04Compass;

		// private void Start
		private void Start()
		{
			// _charger04Compass
			_charger04Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component PoliceCharger04Compass is enabled
				GetComponent<PoliceCharger04Compass>().enabled = true;

				// Debug Log PoliceCharger04 compass is enabled
				//Debug.Log("The PoliceCharger04 compass is enabled");
			...
			{
				// Debug Log PoliceCharger04 compass is disabled
				//Debug.Log("The PoliceCharger04 compass is disabled");
				...
				// Get Component PoliceCharger04Compass is not enabled
				GetComponent<PoliceCharger04Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _policeCharger04 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_policeCharger04.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _policeCharger04 forward vector

			// Vector3 forward is _policeCharger04 transform forward
			Vector3 forward = _policeCharger04.transform.forward;

			// Zero out the Y of _policeCharger04 forward vector to only get the direction in the X,Z 
	...
	} // close public class PoliceCharger04Compass
