/*
*  Name: Swat Van 03 Compass
*  File: SwatVan03Compass.cs
...
namespace NavigationControl
{
	// public class SwatVan03Compass
	public class SwatVan03Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The SwatVan03 transform")]
			// Transform _swatVan03
			[SerializeField] private Transform _swatVan03;
		...
		public static SwatVan03Compass _swatVan03Compass;

		// private void Start
		private void Start()
		{
			// _swatVan03Compass
			_swatVan03Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component SwatVan03Compass is enabled
				GetComponent<SwatVan03Compass>().enabled = true;
			...
			else if (compassEnabled == false)
			{
				...
				// Get Component SwatVan03Compass is not enabled
				GetComponent<SwatVan03Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _swatVan03 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_swatVan03.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _swatVan03 forward vector

			// Vector3 forward is _swatVan03 transform forward
			Vector3 forward = _swatVan03.transform.forward;

			// Zero out the Y of _swatVan03 forward vector to only get the direction in the X,Z 
	...
	} // close public class SwatVan03Compass
