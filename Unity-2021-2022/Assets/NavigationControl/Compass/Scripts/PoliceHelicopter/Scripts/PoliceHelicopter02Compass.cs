/*
*  Name: Police Helicopter 02 Compass
*  File: PoliceHelicopter02Compass.cs
...
namespace NavigationControl
{
	// public class PoliceHelicopter02Compass
	public class PoliceHelicopter02Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The PoliceHelicopter02 transform")]
			// Transform _policeHelicopter02
			[SerializeField] private Transform _policeHelicopter02;
		...
		// PoliceHelicopter02Compass _policeHeli02Compass
		public static PoliceHelicopter02Compass _policeHeli02Compass;

		// private void Start
		private void Start()
		{
			// _policeHeli02Compass
			_policeHeli02Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component PoliceHelicopter02Compass is enabled
				GetComponent<PoliceHelicopter02Compass>().enabled = true;
			...
			else if (compassEnabled == false)
			{
				...
				// Get Component PoliceHelicopter02Compass is not enabled
				GetComponent<PoliceHelicopter02Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _policeHelicopter02 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_policeHelicopter02.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _policeHelicopter02 forward vector

			// Vector3 forward is _policeHelicopter02 transform forward
			Vector3 forward = _policeHelicopter02.transform.forward;

			// Zero out the Y of _policeHelicopter02 forward vector to only get the direction in the X,Z 
	...
	} // close public class PoliceHelicopter02Compass
