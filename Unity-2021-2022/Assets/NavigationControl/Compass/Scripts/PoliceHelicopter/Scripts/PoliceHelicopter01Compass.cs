/*
*  Name: Police Helicopter 01 Compass
*  File: PoliceHelicopter01Compass.cs
...
namespace NavigationControl
{
	// public class PoliceHelicopter01Compass
	public class PoliceHelicopter01Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The PoliceHelicopter01 transform")]
			// Transform _policeHelicopter01
			[SerializeField] private Transform _policeHelicopter01;
		...
		// PoliceHelicopter01Compass _policeHeli01Compass
		public static PoliceHelicopter01Compass _policeHeli01Compass;

		// private void Start
		private void Start()
		{
			// _policeHeli01Compass
			_policeHeli01Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component PoliceHelicopter01Compass is enabled
				GetComponent<PoliceHelicopter01Compass>().enabled = true;
			...
			else if (compassEnabled == false)
			{
				...
				// Get Component PoliceHelicopter01Compass is not enabled
				GetComponent<PoliceHelicopter01Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _policeHelicopter01 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_policeHelicopter01.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _policeHelicopter01 forward vector

			// Vector3 forward is _policeHelicopter01 transform forward
			Vector3 forward = _policeHelicopter01.transform.forward;

			// Zero out the Y of _policeHelicopter01 forward vector to only get the direction in the X,Z 
	...
	} // close public class PoliceHelicopter01Compass
