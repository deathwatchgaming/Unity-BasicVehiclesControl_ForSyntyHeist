/*
*  Name: Police Helicopter 04 Compass
*  File: PoliceHelicopter04Compass.cs
...
namespace NavigationControl
{
	// public class PoliceHelicopter04Compass
	public class PoliceHelicopter04Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The PoliceHelicopter04 transform")]
			// Transform _policeHelicopter04
			[SerializeField] private Transform _policeHelicopter04;
		...
		// PoliceHelicopter04Compass _policeHeli04Compass
		public static PoliceHelicopter04Compass _policeHeli04Compass;

		// private void Start
		private void Start()
		{
			// _policeHeli04Compass
			_policeHeli04Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component PoliceHelicopter04Compass is enabled
				GetComponent<PoliceHelicopter04Compass>().enabled = true;
			...
			else if (compassEnabled == false)
			{
				...
				// Get Component PoliceHelicopter04Compass is not enabled
				GetComponent<PoliceHelicopter04Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _policeHelicopter04 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_policeHelicopter04.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _policeHelicopter04 forward vector

			// Vector3 forward is _policeHelicopter04 transform forward
			Vector3 forward = _policeHelicopter04.transform.forward;

			// Zero out the Y of _policeHelicopter04 forward vector to only get the direction in the X,Z 
	...
	} // close public class PoliceHelicopter04Compass
