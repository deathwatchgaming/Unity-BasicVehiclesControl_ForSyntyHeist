/*
*  Name: Police Helicopter 03 Compass
*  File: PoliceHelicopter03Compass.cs
...
namespace NavigationControl
{
	// public class PoliceHelicopter03Compass
	public class PoliceHelicopter03Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The PoliceHelicopter03 transform")]
			// Transform _policeHelicopter03
			[SerializeField] private Transform _policeHelicopter03;
		...
		// PoliceHelicopter03Compass _policeHeli03Compass
		public static PoliceHelicopter03Compass _policeHeli03Compass;

		// private void Start
		private void Start()
		{
			// _policeHeli01Compass
			_policeHeli03Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component PoliceHelicopter03Compass is enabled
				GetComponent<PoliceHelicopter03Compass>().enabled = true;
			...
			else if (compassEnabled == false)
			{
				...
				// Get Component PoliceHelicopter03Compass is not enabled
				GetComponent<PoliceHelicopter03Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _policeHelicopter03 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_policeHelicopter03.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _policeHelicopter03 forward vector

			// Vector3 forward is _policeHelicopter03 transform forward
			Vector3 forward = _policeHelicopter03.transform.forward;

			// Zero out the Y of _policeHelicopter03 forward vector to only get the direction in the X,Z 
	...
	} // close public class PoliceHelicopter03Compass
