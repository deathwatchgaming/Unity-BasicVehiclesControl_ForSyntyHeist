/*
*  Name: Police Charger 03 Compass
*  File: PoliceCharger03Compass.cs
...
namespace NavigationControl
{
	// public class PoliceCharger03Compass
	public class PoliceCharger03Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The PoliceCharger03 transform")]
			// Transform _policeCharger03
			[SerializeField] private Transform _policeCharger03;
		...
		// PoliceCharger03Compass _charger03Compass
		public static PoliceCharger03Compass _charger03Compass;

		// private void Start
		private void Start()
		{
			// _charger03Compass
			_charger03Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component PoliceCharger03Compass is enabled
				GetComponent<PoliceCharger03Compass>().enabled = true;

				// Debug Log PoliceCharger03 compass is enabled
				//Debug.Log("The PoliceCharger03 compass is enabled");
			...
			{
				// Debug Log PoliceCharger03 compass is disabled
				//Debug.Log("The PoliceCharger03 compass is disabled");
				...
				// Get Component PoliceCharger03Compass is not enabled
				GetComponent<PoliceCharger03Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _policeCharger03 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_policeCharger03.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _policeCharger03 forward vector

			// Vector3 forward is _policeCharger03 transform forward
			Vector3 forward = _policeCharger03.transform.forward;

			// Zero out the Y of _policeCharger03 forward vector to only get the direction in the X,Z 
	...
	} // close public class PoliceCharger03Compass
