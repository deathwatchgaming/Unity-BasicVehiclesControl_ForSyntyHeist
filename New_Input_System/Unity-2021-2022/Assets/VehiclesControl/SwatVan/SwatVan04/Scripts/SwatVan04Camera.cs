/*
 * File: Swat Van 04 Camera
 * Name: SwatVan04Camera.cs
...
namespace VehiclesControl
{
	// public class SwatVan04Camera
	public class SwatVan04Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The Swat Van 04 transform")]
			// Transform _swatVan04
			[SerializeField] private Transform _swatVan04;
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _swatVan04.position.y plus _height
			float _desiredHeight = _swatVan04.position.y + _height;
			...
			// transform position is _swatVan04.position
			transform.position = _swatVan04.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _swatVan04.InverseTransformDirection(_swatVan04.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _swatVan04.eulerAngles.y plus 180
				_tempVector3.y = _swatVan04.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _swatVan04.eulerAngles.y
				_tempVector3.y = _swatVan04.eulerAngles.y;
			...
			float _autoCamControl = _swatVan04.GetComponent<Rigidbody>().velocity.magnitude;
	...
	} // close public class SwatVan04Camera
