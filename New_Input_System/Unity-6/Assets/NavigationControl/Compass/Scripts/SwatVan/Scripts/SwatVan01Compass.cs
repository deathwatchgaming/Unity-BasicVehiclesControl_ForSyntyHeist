/*
*  Name: Swat Van 01 Compass
*  File: SwatVan01Compass.cs
...
namespace NavigationControl
{
	// public class SwatVan01Compass
	public class SwatVan01Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The SwatVan01 transform")]
		...
		public static SwatVan01Compass _swatVan01Compass;

		// private void Start
		private void Start()
		{
			// _swatVan01Compass
			_swatVan01Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component SwatVan01Compass is enabled
				GetComponent<SwatVan01Compass>().enabled = true;
			...
			else if (compassEnabled == false)
			{
				...
				// Get Component SwatVan01Compass is not enabled
				GetComponent<SwatVan01Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _swatVan01 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_swatVan01.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _swatVan01 forward vector

			// Vector3 forward is _swatVan01 transform forward
			Vector3 forward = _swatVan01.transform.forward;

			// Zero out the Y of _swatVan01 forward vector to only get the direction in the X,Z 
	...
	} // close public class SwatVan01Compass
