/*
 * File: Swat Van 02 Camera
 * Name: SwatVan02Camera.cs
...
namespace VehiclesControl
{
	// public class SwatVan02Camera
	public class SwatVan02Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The Swat Van 02 transform")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _swatVan02.position.y plus _height
			float _desiredHeight = _swatVan02.position.y + _height;
			...
			// transform position is _swatVan02.position
			transform.position = _swatVan02.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _swatVan02.InverseTransformDirection(_swatVan02.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _swatVan02.eulerAngles.y plus 180
				_tempVector3.y = _swatVan02.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _swatVan02.eulerAngles.y
				_tempVector3.y = _swatVan02.eulerAngles.y;
			...
			float _autoCamControl = _swatVan02.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class SwatVan02Camera
