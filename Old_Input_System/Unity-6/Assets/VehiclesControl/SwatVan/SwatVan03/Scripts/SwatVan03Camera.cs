/*
 * File: Swat Van 03 Camera
 * Name: SwatVan03Camera.cs
...
namespace VehiclesControl
{
	// public class SwatVan03Camera
	public class SwatVan03Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The Swat Van 03 transform")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _swatVan03.position.y plus _height
			float _desiredHeight = _swatVan03.position.y + _height;
			...
			// transform position is _swatVan03.position
			transform.position = _swatVan03.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _swatVan03.InverseTransformDirection(_swatVan03.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _swatVan03.eulerAngles.y plus 180
				_tempVector3.y = _swatVan03.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _swatVan03.eulerAngles.y
				_tempVector3.y = _swatVan03.eulerAngles.y;
			...
			float _autoCamControl = _swatVan03.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class SwatVan03Camera
