/*
*  Name: Swat Van 02 Compass
*  File: SwatVan02Compass.cs
...
namespace NavigationControl
{
	// public class SwatVan02Compass
	public class SwatVan02Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The SwatVan02 transform")]
			// Transform _swatVan02
			[SerializeField] private Transform _swatVan02;
		...
		public static SwatVan02Compass _swatVan02Compass;

		// private void Start
		private void Start()
		{
			// _swatVan02Compass
			_swatVan02Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component SwatVan02Compass is enabled
				GetComponent<SwatVan02Compass>().enabled = true;
			...
			else if (compassEnabled == false)
			{
				...
				// Get Component SwatVan02Compass is not enabled
				GetComponent<SwatVan02Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _swatVan02 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_swatVan02.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _swatVan02 forward vector

			// Vector3 forward is _swatVan02 transform forward
			Vector3 forward = _swatVan02.transform.forward;

			// Zero out the Y of _swatVan02 forward vector to only get the direction in the X,Z 
	...
	} // close public class SwatVan02Compass
